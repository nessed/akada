/**
 * Markd's note format, written once. The copy-paste prompt wraps it for a
 * chat that cannot reach Akada; the MCP tools hand it to an assistant that
 * can, so a note written either way reads the same on the page.
 */
export const FORMAT_RULES = `FORMAT RULES
